...
\item \textsf{\textbf{I need to add instructions here on how to setup Unsecure-12 in HiWave debugger to actually perform the unsecure task.}}
\begin{itemize}
	\item \textsf{Search for and create a shortcut on your Desktop for hiwave.exe, then open the shortcut to hiwave.exe that you just created.} \newline
		  \textsf{The defualt path to hiwave.exe on a 32bit system is:} \newline
	      \PVerb{C:\Program Files\Freescale\CWS12v5.1\Prog\hiwave.exe} \newline
	      \textsf{The defualt path to hiwave.exe on a 64bit system is:} \newline
	...
	\item \textsf{Click on the \textbf{Startup} tab, then click the \textbf{Browse...} button and locate the} \newline
              \textsf{\textbf{P\&E\_ICD\_Startup.cmd} file, click the \textbf{Open} button, then click the checkbox for \textbf{Enable Command File}.}
              
	\item \textsf{Click on the \textbf{Reset} tab, then click the \textbf{Browse...} button and locate the} \newline
              \textsf{\textbf{P\&E\_ICD\_Reset.cmd} file, click \textbf{Open} button, then click the checkbox for \textbf{Enable Command File}.}
              
	\item \textsf{Click on the \textbf{Preload} tab, then click the \textbf{Browse...} button and locate the} \newline
              \textsf{\textbf{P\&E\_ICD\_Preload.cmd} file, click \textbf{Open} button, then click the checkbox for \textbf{Enable Command File}.}
              
	\item \textsf{Click on the \textbf{Postload} tab, then click the \textbf{Browse...} button and locate the} \newline
              \textsf{\textbf{P\&E\_ICD\_Postload.cmd} file, click \textbf{Open} button, then click the checkbox for \textbf{Enable Command File}.}
              
	\item \textsf{Click on the \textbf{Vppon} tab, then click the \textbf{Browse...} button and locate the} \newline
              \textsf{\textbf{P\&E\_ICD\_Vppon.cmd} file, click \textbf{Open} button, then click the checkbox for \textbf{Enable Command File}.}
              
	\item \textsf{Click on the \textbf{Vppoff} tab, then click the \textbf{Browse...} button and locate the} \newline
              \textsf{\textbf{P\&E\_ICD\_Vppoff.cmd} file, click \textbf{Open} button, then click the checkbox for \textbf{Enable Command File}.}
              
	\item \textsf{Click on the \textbf{Unsecure} tab, then click the \textbf{Browse...} button and locate the} \newline
              \textsf{\textbf{P\&E\_ICD\_Erase\_unsecure\_hcs12.cmd} file, click \textbf{Open} button, then click the checkbox for \textbf{Enable Command File}.}
              
    \item \textsf{Click on the \textbf{OK} button to save your changes.}
\end{itemize}

\item \textsf{Once you have completed all the Windows 7 updates and reboots proceed to Step 2 in the next chapter.}
\end{enumerate}

\chapter{\textsf{Step 2: Download and create boot media}} 
\textsf{Download and create the appropriate boot media for either Debian 6 (Squeeze) or Ubuntu 11.10 (Oneiric Ocelot).
